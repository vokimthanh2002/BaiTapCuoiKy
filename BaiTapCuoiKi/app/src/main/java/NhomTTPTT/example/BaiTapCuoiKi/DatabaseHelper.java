...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void insertTaiKhoan(Account tk){
    ...
    public void updateProfile(Account account){
        db=this.getWritableDatabase();
        ContentValues values= new ContentValues();
        values.put(COLUMN_FULLNAME,account.getFullname());
        values.put(COLUMN_EMAIL,account.getEmail());
        values.put(COLUMN_USERNAME,account.getUserName());
        db.update(TABLE_NAME,values,COLUMN_PHONE+"= ?",new String[] {String.valueOf(account.getPhone())});
        db.close();
    }
